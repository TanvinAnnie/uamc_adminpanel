...
const normalizeCampusLifeData = (
  initialData?: CampusLifeFormData | null
): CampusLifeFormData => {
  if (!initialData) {
    return {
      ...defaultFormData,
      items: defaultFormData.items.map((item) => ({ ...item })),
    };
  }
...
export default function CampusLifeForm({
  ...
  const [
    ...
    normalizeCampusLifeData(initialData)
  );

  // Keep the form synchronized when the edit page finishes
  // loading data after the component has already mounted.
  useEffect(() => {
    if (initialData) {
      setFormData(
        normalizeCampusLifeData(initialData)
  ...
  const handleSubmit = async (
    ...
    try {
      ...
      const payload: CampusLifeFormData = {
        ...formData,
