...
export default function CampusLifeNewPage() {
  ...
  const handleCreate = async (data: CampusLifeFormData) => {
    try {
      ...
      let result: CampusLifeCreateResponse | null = null;

      try {
        result = responseText
          ? (JSON.parse(responseText) as CampusLifeCreateResponse)
          : null;
      } catch {
        console.error(
      ...
      if (!response.ok || !result?.success) {
        throw new Error(
          result?.message || "Failed to create Campus Life."
        );
      }

      toast.success("Campus Life created successfully.");

      router.push("/dashboard/home/campus-life");
      router.refresh();
    } catch (error) {
      ...
      <div className="mb-6 flex flex-col gap-4">
        ...
        <Link
          ...
          className="
            ...
            hover:bg-slate-50 hover:text-[#008B45]
          "
        >
          <ArrowLeft size={17} />
          Back to Campus Life
        ...
        <div>
          ...
          <p
            ...
            "
          >
            Add and manage the Campus Life content that will appear on
            the client website.
          </p>
        </div>
      </div>
